...
async function replay(mode: "normal" | "drift" | "cleanup" | "denied" | "partial" | "accounting" | "intermediate") {
  ...
  const replayActions = mode === 'intermediate' ? [...actions.slice(0, -1), { kind: 'observe' as const, terminal: false }, { kind: 'practice-start' as const }, { kind: 'observe' as const, terminal: true }] : actions;
  ...
  let ref = 0, practiceStarted = false, observations = 0;
  ...
  if (["normal", "partial", "intermediate"].includes(mode)) await run({ request }); else await assert.rejects(run({ request }));
...
async function main() { for (const mode of ["normal", "drift", "cleanup", "denied", "partial", "accounting", "intermediate"] as const) await replay(mode); console.log("Generated native replay: credential substitution, identity rejection, paid permission and mandatory cleanup pass"); }
