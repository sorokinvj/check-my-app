...
// coordinate. It is consumed before input so a slow/retried request cannot
// toggle a paid control twice or reuse a node after the popup has changed.
export class NativeSurface {
  constructor(invoke, now = Date.now) { this.invoke = invoke; this.now = now; this.snapshot = null; this.credentialFilled = false; this.credentialSubmitted = false; }
